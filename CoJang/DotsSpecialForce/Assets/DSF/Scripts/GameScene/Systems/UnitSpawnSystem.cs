...
public partial struct UnitSpawnSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        if (spawnCount < spawnLimit)
        {
            ...
            if (currentTime > intervalTime)
            {
                ...
                ecb.AddComponent<ChaserComponent>(entity);
                ecb.SetComponent(entity, new ChaserComponent { HP = 1 });

                ecb.AddComponent<DestoryTag>(entity);
                ecb.SetComponent(entity, new DestoryTag { IsDestoryed = false, self = entity });
    ...
    private void OnCtrlClick(InputAction.CallbackContext context)
    {
        ...
        if (Raycaster.ShootRay())
        {
            ...
            entityManager.AddComponent<ChaserComponent>(entity);
            entityManager.SetComponentData(entity, new ChaserComponent { HP = 3 });
