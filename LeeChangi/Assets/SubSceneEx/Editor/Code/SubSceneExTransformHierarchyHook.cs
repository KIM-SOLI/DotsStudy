...
{
    [InitializeOnLoad]
    class SubSceneExTransformHierarchyHook
    {
        static SubSceneExTransformHierarchyHook()
        {
            ...
            EditorApplication.hierarchyWindowItemOnGUI += SubSceneExHierarchyOverlay.HierarchyWindowItemOnGUI; 
        }
        ...
        {
            var scenes = new SceneHierarchyHooks.SubSceneInfo[SubSceneEx.AllSubScenes.Count()];
            var sceneAssets = new HashSet<SceneAsset>();

            int index = 0;
            foreach (var subScene in SubSceneEx.AllSubScenes)
            {
