...
namespace Unity.Scenes.Editor
{
    ...
    [CustomEditor(typeof(SubSceneEx))]
    [CanEditMultipleObjects]
    public class SubSceneExEditor : UnityEditor.Editor
    {
        SceneAsset[] m_PreviousSceneAssets;
        private SubSceneEx[] _selectedSubscenes;

        private void OnEnable()
        {
            Undo.undoRedoPerformed += OnUndoRedoPerformed;
            _selectedSubscenes = new SubSceneEx[targets.Length];
            var targetsArray = targets;
            targetsArray.CopyTo(_selectedSubscenes, 0);

            SubSceneExInspectorUtility.WantsRepaint += this.Repaint;
        }
        ...
        void OnUndoRedoPerformed()
        {
            ...
            // reload the Hierarchy which depends on the current SubSceneEx state.
        ...
        void CachePreviousSceneAssetReferences()
        {
            ...
            {
                var SubSceneEx = (SubSceneEx)targets[i];
                m_PreviousSceneAssets[i] = SubSceneEx.SceneAsset;
            }
        }
        ...
        void HandleChangedSceneAssetReferences()
        {
            ...
            for (int i = 0; i < numTargets; ++i)
            {
                ...
                if (prevSceneAsset != SubSceneEx.SceneAsset)
                {
                    if (!needsHierarchyReload)
                    {
                        ...
                        var scene = SubSceneEx.EditingScene;
        ...
        private static void DrawOpenSubScenes(SubSceneEx[] subScenes)
        {
            int numOpen = 0;
            ...
            if (numOpen > 0)
            {
                ...
                for (int i = 0; i < subScenes.Length; i++)
                {
                    ...
                    using (new EditorGUI.DisabledScope(!scene.EditingScene.isDirty))
                    {
                        ...
                        {
                            SubSceneExInspectorUtility.SaveScene(scene);
                        }
                    }
                    buttonRect.x += buttonRect.width + spacing;
                    if (GUI.Button(buttonRect, Content.CloseLabel))
                    {
                        SubSceneExInspectorUtility.CloseAndAskSaveIfUserWantsTo(scene);
                    }
                }
            }
        }

        private static bool DrawClosedSubScenes(SubSceneExInspectorUtility.LoadableScene[] loadableScenes, SubSceneEx[] subscenes)
        {
            if (World.DefaultGameObjectInjectionWorld != null && loadableScenes.Length != 0)
            {
...

                {
                    ...
                    foreach (var scene in loadableScenes)
                    {
                        if (entityManager.HasComponent<RequestSceneLoaded>(scene.Scene))
                            numScenesLoaded++;
                        if (IsSubsceneImported(scene.SubSceneEx))
                            numScenesImported++;
                    }
                ...
                if (loadableScenes.Length > 1)
                {
                    ...
                    if (reimportRequested && EditorUtility.DisplayDialog(Content.ReimportAllSubScenes, Content.ReimportAllSubScenesDetails, Content.Yes, Content.No))
                        SubSceneExInspectorUtility.ForceReimport(subscenes);
                ...
                foreach (var scene in loadableScenes)
                {
                    ...
                    s_TmpContent.tooltip = scene.SubSceneEx.EditableScenePath;
                    ...
                    {
                        SubSceneExInspectorUtility.ForceReimport(scene.SubSceneEx);
                    }
                    ...
                    buttonRect.x += buttonRect.width + spacing;
                    using (new EditorGUI.DisabledScope(!SubSceneExInspectorUtility.CanEditScene(scene.SubSceneEx)))
                    {
                        if (GUI.Button(buttonRect, Content.OpenLabel))
                        {
                            SubSceneExUtility.EditScene(scene.SubSceneEx);
                        }
                    }
                }
        ...
        {
            var SubSceneEx = target as SubSceneEx;

            if (!SubSceneEx.IsInMainStage())
            {
                // In Prefab Mode and when selecting a Prefab Asset in the Project Browser we only show the inspector of data of the
                // SubSceneEx, and not the load/unload/edit/close buttons.
                base.OnInspectorGUI();
            ...
            var prevColor = SubSceneEx.HierarchyColor;
            var prevAutoLoad = SubSceneEx.AutoLoadScene;
            ...
            if (SubSceneEx.HierarchyColor != prevColor)
                SceneHierarchyHooks.ReloadAllSceneHierarchies();

            if (prevAutoLoad != SubSceneEx.AutoLoadScene)
                SubSceneEx.RebuildSceneEntities();

            DrawOpenSubScenes(_selectedSubscenes);
            var loadableScenes = SubSceneExInspectorUtility.GetLoadableScenes(_selectedSubscenes);
            ...
            if (GUILayout.Button("ClearWorld"))
            {
                ...
                var scenes = FindObjectsOfType<SubSceneEx>();
            ...
            bool hasDuplicates = SubSceneEx.SceneAsset != null && (SubSceneEx.AllSubScenes.Count(s => (s.SceneAsset == SubSceneEx.SceneAsset)) > 1);
            if (hasDuplicates)
            {
                EditorGUILayout.HelpBox($"The Scene Asset '{SubSceneEx.EditableScenePath}' is used mutiple times and this is not supported. Clear the reference.", MessageType.Warning, true);
                if (GUILayout.Button("Clear"))
                {
                    SubSceneEx.SceneAsset = null;
                    SceneHierarchyHooks.ReloadAllSceneHierarchies();
                }
                EditorGUILayout.Space();
            }

            var uncleanHierarchyObject = SubSceneExInspectorUtility.GetUncleanHierarchyObject(_selectedSubscenes);
            ...
            if (SubSceneExInspectorUtility.HasChildren(_selectedSubscenes))
            {
        ...
        bool HasFrameBounds()
        {
            return !SubSceneExInspectorUtility.GetActiveWorldMinMax(World.DefaultGameObjectInjectionWorld, targets).Equals(MinMaxAABB.Empty);
        }

        Bounds OnGetFrameBounds()
        {
            AABB aabb = SubSceneExInspectorUtility.GetActiveWorldMinMax(World.DefaultGameObjectInjectionWorld, targets);
            return new Bounds(aabb.Center, aabb.Size);
        }

        // Visualize SubSceneEx using bounding volume when it is selected.
        [DrawGizmo(GizmoType.Selected)]
        static void DrawSubsceneBounds(SubSceneEx scene, GizmoType gizmoType)
        {
            SubSceneExInspectorUtility.DrawSubsceneBounds(scene);
        }

        static bool IsSubsceneImported(SubSceneEx SubSceneEx)
        {
            foreach (var world in World.All)
            {
                ...
                {
                    var hash = EntityScenesPaths.GetSubSceneArtifactHash(SubSceneEx.SceneGUID, world.EntityManager.GetComponentData<SceneSystemData>(sceneSystem).BuildConfigurationGUID, true, ImportMode.NoImport);
                    if (!hash.IsValid)
                        return false;
                }
            }
